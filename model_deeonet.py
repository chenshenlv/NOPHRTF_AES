...
class DeepONet(nn.Module):
    ...
    def __init__(
        ...
        self.branch_cnn = self.build_branch_cnn(layer_sizes_branch, "dgcnn")
        self.branch_merge = self.build_branch_merge(layer_sizes_branch)
        self.trunk = self.build_trunk(layer_sizes_trunk, "Fourier_UFNN")
    ...
    def build_branch_cnn(self, branch_config, cnn_type):
        ...
        if cnn_type == "pointmsg":
            cnn_config = branch_config["point"]
            cnn = PointNetmsg(latent_size=cnn_config["latent_size"])
    ...
    def build_branch_merge(self,branch_config):
        ...
        fnn = FNN(layer_sizes=[input_size,512,output_size], activation=self.activation_branch,dropout=0.2,use_batchnorm=True)
        return fnn
    ...
    def forward(self, inputs):
        ...
        x_loc = torch.reshape(x_loc,(-1,3))

        x_func = self.branch_cnn(x_pts)
        x_func = torch.concatenate([x_func,scale],dim=1)
        x_func = self.branch_merge(x_func)
        
        x_trunk = self.trunk(x_loc)
        x_trunk = x_trunk.reshape(n_batch,n_eval,-1)

        # using decoder
        if self.decoder_enable: 
            x = torch.einsum("bi,bni->bni", x_func, x_trunk)
