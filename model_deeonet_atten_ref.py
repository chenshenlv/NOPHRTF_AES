...
class DeepONet(nn.Module):
    ...
    def __init__(
        ...
        self.ref_encoder = self.build_ref_encoder(layer_sizes_branch)
    ...
    def build_ref_encoder(self,branch_config):
        ...
        fnn = FNN(layer_sizes=[input_size,128,output_size], activation=self.activation_trunk)
        return fnn
    ...
    def forward(self, inputs):
        ...
        # compute r = loc - src_l/r
        src_l_exp = src_l.unsqueeze(1).repeat(1,n_eval,1)  # (B,L,3)
        r_l = x_loc - src_l_exp  # (B,L,3)
        d_rl = torch.norm(r_l, dim=2, keepdim=True)  # (B,L,1)
        ...
        # x_trunk = torch.cat([x_trunk.reshape(n_batch, n_eval, -1),x_loc.reshape(n_batch, n_eval, -1),src_l_], dim=-1)
        x_trunk = torch.cat([x_trunk.reshape(n_batch, n_eval, -1),x_loc], dim=-1)
        ...
        x_latent = self.cross_atten(x_func.reshape(n_batch,-1,n_pts), x_trunk.reshape(n_batch, n_eval, -1), x_pts[:,:3,:], cond)
        # print(torch.norm(x_pts[0,:3,0]))
        # print(torch.norm(x_pts[0,3:,0] ))
        ref_enc = self.ref_encoder(ref.reshape(n_batch*n_eval,-1))
        x_latent = torch.einsum("bi,bi->bi", x_latent , ref_enc)
        # using decoder
