...
class VAE():
    ...
    def build_data(self, train: bool = True):
        ...
        if train:
            ...
            if self.args.standardize:
                ...
                train_array_h = train_array_h[:, :, :, 1].reshape(n_train*n_loc, n_freq)
                val_array_h = val_array_h[:, :, :, 1].reshape(n_val*n_loc, n_freq)
            else:
        ...
        if not train: #test
            ...
            if self.args.standardize:
                ...
                test_array_h = test_array_h[:, :, :, 1].reshape(n_test*n_loc, n_freq)
            else:
    ...
    def train(self,):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            if val_loss < best_val:
                ...
                torch.save(ckpt, os.path.join(self.args.output_dir, "ae_fnn.pt"))
    ...
    def test(self,):
        ...
        ckpt_path = os.path.join(self.args.output_dir, "ae_fnn.pt")
        ...
        if self.args.standardize and scaler is not None:
            ...
            y_pred_2 = torch.stack((zeros, y_pred), dim=-1).to(self.device)
            y_true_2 = torch.stack((zeros, y_true), dim=-1).to(self.device)
            y_pred_inv = scaler.inverse_hrtf(
                y_pred_2, n_sub=n_test, n_loc=n_loc, n_freq=n_freq, n_dim=2
            )
            y_true_inv = scaler.inverse_hrtf(
                y_true_2, n_sub=n_test, n_loc=n_loc, n_freq=n_freq, n_dim=2
            )
            lse_loss = LSD_l_mag_db(
                y_true_inv[..., 1].cpu().numpy(), y_pred_inv[..., 1].cpu().numpy()
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--train-data",
        default="./model_point/data/AXD/dis_weighted/mag/raw/msr_44_ff/5k/y_train.sav",
        help="Path to .npy/.npz data.",
    )
    parser.add_argument("--data-key", default=None, help="NPZ key for train data.")
    parser.add_argument("--val-data", default="./model_point/data/AXD/dis_weighted/mag/raw/msr_44_ff/5k/y_val.sav", help="Optional .npy/.npz/.sav val data.")
    parser.add_argument("--val-key", default=None, help="NPZ key for val data.")
    parser.add_argument("--val-ratio", type=float, default=0.1)
    parser.add_argument("--test-data", default="./model_point/data/AXD/dis_weighted/mag/raw/msr_44_ff/5k/y_test.sav", help="Optional .npy/.npz/.sav val data.")
