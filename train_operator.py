...
from DataLoader_operator import Dataloader
from model_deeonet import DeepONet
...
class ModelLowFTrainer():
    ...
    def __init__(self, arg_config, device=None):
        ...
        self.epochs = self.arg_config.get("epoch",100)
    ...
    def _build_test_data(self):
        x_test = joblib_load(self.data_save_folder+"X_val.sav")
        y_test = joblib_load(self.data_save_folder+"y_val.sav")
    ...
    def _load_AE(self,input_dim):
        ckpt_path = os.path.join(self.ae_path, "ae_fnn.pt")
    ...
    def train(self, 
        ...
        y_train_d = torch.from_numpy(y_train).to(device=self.device, dtype=torch.float32, non_blocking=True)
        y_val_d   = torch.from_numpy(y_val).to(device=self.device, dtype=torch.float32, non_blocking=True)

        if not use_saved_scalar:
            self.scaler = Scaler(
                y=y_train_d,itd=None,
                hrtf_scaler_path="./Scaler/hrtf_scaler_temp",
                force_recompute=True,
                )
            self.scaler.init_scalers(True)

        if use_saved_scalar:
            self.scaler = Scaler(
                y=None,itd=None,
                hrtf_scaler_path=self.hrtf_scaler_path,
                )
            self.scaler.init_scalers()
        
        
        point_train = X_train[1]
        X_train = (x_fre,point_train,X_train[2],X_train[3][0:n_train,:],X_train[4][0:n_train,:])
        point_val = X_val[1]
        X_val = (x_fre,point_val,X_val[2],X_val[3],X_val[4])

        y_train_d = self.scaler.normalize_hrtf(y_train_d).reshape(n_train, n_loc, n_freq, -1)
        y_val_d = self.scaler.normalize_hrtf(y_val_d).reshape(n_test,n_loc, n_freq, -1)
        y_train_h = y_train_d.cpu().numpy()
        y_val_h = y_val_d.cpu().numpy()
        y_train_h = y_train_h[:,:,:,1].reshape(n_train*n_loc, -1)
        y_val_h = y_val_h[:,:,:,1].reshape(n_test*n_loc,-1)

        assert np.isfinite(y_train).all(), f"hrtf array contains {np.count_nonzero(~np.isfinite(y_train[0]))} invalid entries"
        data = self._build_data(X_train, y_train_h, X_val, y_val_h)
        self._build_net()
        ...
        best_val = float("inf")
        total_iterations = (n_train*n_loc)//(self.batch_size[0]*self.batch_size[1])
        alpha_latent = 1.0
        beta_recon   = 1.0
        for epoch in range(1, self.epochs + 1):
            ...
            with torch.no_grad():
                ...
                y_rd_hat = self.op_net(inputs)
                y_rec = self.ae_net.decoder(y_rd_hat)
                # reconstruction loss
                loss = F.mse_loss(y_rec, y_test, reduction="mean")
                val_loss = loss.item()
                # latent loss
                y_rd = self.ae_net.encoder(y_test)
                loss_latent = F.mse_loss(y_rd_hat, y_rd, reduction="mean")
                val_latent = loss_latent.item()
                scheduler.step(val_loss)
            #     val_loss += loss.item() * (n_test*n_loc)
            # val_loss /= (n_test*n_loc)

            if val_loss < best_val:
                print(f'val improve from {best_val} to {val_loss}')
                best_val = val_loss
            ...
            print(
                ...
                | val_recon={val_loss:.6f}, val_latent={val_latent:.6f} | lr={optimizer.param_groups[0]['lr']:.6f} "
            )

    
    def test(self, model_name):
        x_test, y_test = self._build_test_data()
        ...
        with ctx:
            for s in range(0, N, batch_size):
                ...
                try:
                    y_rd_hat_b = self.op_net(*inputs_b)
        ...
        # calculate the lsd
        LSD_loss_l = LSD_l_mag_db(y_pred[...,0],y_test[...,0])
        LSD_loss_r = LSD_l_mag_db(y_pred[...,1],y_test[...,1])
...
def run_train(cfg, use_saved_scalar, resume):
    ...
    try:
        ...
        trainer.train(
                     use_saved_scalar=use_saved_scalar,
                     resume_train=resume)
    finally:
        del trainer
        gc.collect()        
...
def run_test(cfg, model_name):
    trainer = None
    try:
        trainer = ModelLowFTrainer(cfg)
        trainer.test(model_name)
    finally:
        del trainer
        gc.collect()        
        if torch.cuda.is_available():
...
if __name__ == "__main__":
    ...
    for pt in pt_size:
        ...
        if test:
            ...
            run_test(arg_config,model_name)
        ...
        if prediction:
            ...
            "model save path": "./model_point/trained_model/disweighted",
            # "model save path": "./model_point/trained_model/disweighted",
            # "model name": "model_point_dense_" + pt,
            "model name": "model_point_single"+ "UHM" +"_"+ "simu"+ "_" + pt,
            # "data save folder": "./model_point/data/dis_weighted/" + pt + "_dense" +"/" + "recons/",
            "data save folder": "./model_point/data/"+ database +"/dis_weighted/mag/raw/"+datatype+"/" + pt +"/",
            # "anthro scaler path": "./Scaler/scaler_point_" + pt + "_UHM_dense_train",
            # "anthro scaler path": "./Scaler/scaler_point_" + pt + "_" +database+"_isotropic",
            "anthro scaler path": "./Scaler/anthro_scaler_temp",
            # "hrtf scaler path": "./Scaler/scaler_hrtf_UHMsimu",
            # "hrtf scaler path": "./Scaler/scaler_hrtf_"+ database + datatype,
            "hrtf scaler path": "./Scaler/hrtf_scaler_temp", 
            "temp model path": "model_point_single"+ database +"_"+ datatype+ "_" + pt +".pt",
            # "results path": "./model_point/result/dis_weighted/" + pt + "_dense" +"/" + "recons/",
            "results path": "./model_point/result/"+ database +"/dis_weighted/" + pt + "/",
            "decoder_enable": decoder_enable,
            }
            model_name = arg_config["model name"] + ".pt"
            run_prediction(arg_config,model_name)
