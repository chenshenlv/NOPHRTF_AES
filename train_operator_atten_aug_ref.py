...
from DataLoader_operator import Dataloader_augment_ref,Preprocessor
...
class ModelLowFTrainer():
    """
    Trainer for model with reference P_0 magnitude as input.
    """
    ...
    def _build_data(self, X_train, y_train, X_val, y_val):
        return Dataloader_augment_ref(X_train, y_train, X_val, y_val,n_resample=self.n_resample,sigma=self.sigma)
    ...
    def train(self):
        ...
        n_ex = 900
        X_train = (X_train[0],X_train[1][0:n_ex],X_train[2],X_train[3][0:n_ex],X_train[4][0:n_ex],X_train[5][0:n_ex])
    ...
    def test(self, model_name, ):
        ...
        x_fre,x_pt,x_cent, x_eval,x_src_l,x_src_r,x_ref = preprocessor.preprocess_inputs(x_test)
        n_sub = x_pt.shape[0]
        n_eval_loc = x_eval.shape[0] ## x[3] has 2*n_loc
        n_freq = x_fre.shape[0]
        y_test = preprocessor.preprocess_outputs(x_eval, y_test)
        ...
        y_pred_t = y_pred_t.reshape(n_sub,n_eval_loc,n_freq,1)
...
if __name__ == "__main__":
    ...
    for pt in pt_size:
        ...
        "data save folder": "./model_point/data/"+ database +"/AE/mag/raw/"+datatype+"/" + pt +"/",
        ...
        "head resampling num": 5000,
        "sigma":0.6,
        "temp model path": "model_point_single"+ database +"_"+ datatype+ "_" + pt +".pt",
        "results path": "VAE/result/"+ database +"/AE/" + pt + "/",
        "epochs": 200,
